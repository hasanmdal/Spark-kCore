...
object KTrussPregel{
  ...
  def mergeMsgTruss(msg1: String, msg2:String): String = msg1+":"+msg2

  def vprogTruss(vertexId: VertexId, value: (Vert,Int,Int), message: String): (Vert,Int,Int) = {
    if (message == initialMsg){
      ...
      {
        return (value._1,newTrussness,cTruss)
      }
      return (value._1,value._2,value._2)
    }
  }
  ...
  def sendMsgTruss(triplet: EdgeTriplet[(Vert, Int , Int), Long]): Iterator[(VertexId, String)] = {
  ...
  def main(args: Array[String]){
    ...
    val conf: SparkConf = new SparkConf().setAppName("KTrussPregel")
    ...
    val vertLineGraph:VertexRDD[(Vert,Int,Int)]= VertexRDD(mEdges.map(edge=>{(Vert(edge.srcId.toLong, edge.dstId.toLong) , edge.attr._1+2 , 100000)}).zipWithIndex.map(vertex=>{(vertex._2,vertex._1)}))
    ...
    val lineGraph= Graph(vertLineGraph, edgeLineGraph, (Vert(0L , 0L) , 2 , 10000000) , StorageLevel.MEMORY_AND_DISK , StorageLevel.MEMORY_AND_DISK).removeSelfEdges().groupEdges((e1, e2) => e1).cache()
